...
export function Modal(
    ...
    return <div className="fixed inset-0 z-10 w-full h-full bg-neutral-900/80 flex items-center justify-center">
        <div className="w-[350px] bg-neutral-800 shadow-xl shadow-neutral-900 flex flex-col items-center rounded-sm p-4 gap-4">
            ...
            <div className="w-full flex justify-between">
                <button type="submit" onClick={() => cancelCallback()} className="text-stone-200 bg-neutral-900 rounded-[4px] text-sm hover:bg-[#1a1a1a] px-5 py-2 cursor-pointer">Cancel</button>
                <button type="submit" onClick={() => confirmCallback()} className="text-stone-200 bg-neutral-900 rounded-[4px] text-sm hover:bg-[#1a1a1a] px-5 py-2 cursor-pointer">{confirmLabel}</button>
            </div>
        </div>
    </div>
}
